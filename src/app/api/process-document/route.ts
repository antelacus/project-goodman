...
export async function GET(req: NextRequest) {
  try {
    ...
    const documentInfo = {
      id: document.id,
      name: document.name,
      summary: document.document_summaries?.[0] ? {
        document_type: document.document_summaries[0].document_type,
        summary: document.document_summaries[0].summary,
        key_metrics: document.document_summaries[0].key_metrics,
        time_period: document.document_summaries[0].time_period,
      } : null,
      ...
      totalChunks: document.document_chunks?.length || 0
    };
    ...
    const errorMessage = err instanceof Error ? err.message : "未知错误";
    return NextResponse.json(
      { error: "Failed to get document", details: errorMessage },
      { status: 500 }
    );
  }
} 